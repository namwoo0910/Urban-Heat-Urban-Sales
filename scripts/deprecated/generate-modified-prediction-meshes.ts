...
/**
 * Modified AI Prediction Mesh Generator
 * Generates binary mesh files for temperature scenarios T+5, T+15, T+20
 * from 2024-07_simulated_modified.json
 */
...
const MODIFIED_PREDICTION_FILE = path.join(__dirname, '../public/data/prediction/2024-07_simulated_modified.json')
const OUTPUT_DIR = path.join(__dirname, '../public/data')
const BINARY_OUTPUT_DIR = path.join(__dirname, '../public/data/binary/prediction')

// Temperature scenarios for modified data (excluding T+10 which is already done)
const TEMPERATURE_SCENARIOS = [
  { key: 't050', field: '총매출액_(T=5)', label: 'T+5°C' },
  { key: 't150', field: '총매출액_(T=15)', label: 'T+15°C' },
  { key: 't200', field: '총매출액_(T=20)', label: 'T+20°C' }
]
...
/**
 * Convert JSON mesh to binary format with color data
 */
function convertToBinary(
  ...
  const header = {
    ...
    colorsBytes: meshData.colors?.byteLength || 0,  // Include colors byte length
  ...
  // Create binary buffer (includes colors)
  const totalSize =
  ...
  // Write colors (IMPORTANT: This adds gradient color data)
  if (meshData.colors) {
    Buffer.from(meshData.colors.buffer).copy(buffer, offset)
    offset += meshData.colors.byteLength
    console.log(`  Color data included: ${meshData.colors.byteLength} bytes`)
  }
...
function generatePredictionMesh(
  ...
  if (meshData.colors) {
    console.log(`  Colors generated: ${meshData.colors.length / 4} vertices with RGBA`)
  } else {
    console.warn('  ⚠️ Warning: No color data generated!')
  }
...
/**
 * Process modified predictions for all 31 days
 */
async function processModifiedPredictions(districtFeatures: any[]): Promise<void> {
  console.log('\n========================================')
  console.log('Processing Modified Predictions')
  console.log('Temperature Scenarios: T+5°C, T+15°C, T+20°C')
  console.log('========================================')

  if (!fs.existsSync(MODIFIED_PREDICTION_FILE)) {
    console.error(`Modified prediction file not found: ${MODIFIED_PREDICTION_FILE}`)
    return
  }

  const predictions: PredictionData[] = JSON.parse(fs.readFileSync(MODIFIED_PREDICTION_FILE, 'utf8'))
  console.log(`Loaded ${predictions.length} modified prediction records`)
  ...
  // Process each date (starting from July 7th since 1-6 are done)
  const startDate = '2024-07-07'
  const filteredDates = dates.filter(d => d >= startDate)
  console.log(`\nContinuing from ${startDate}...`)

  for (const date of filteredDates) {
    console.log(`\n📅 Processing date: ${date}`)
    const dateCode = date.replace(/-/g, '') // 2024-07-01 -> 20240701

    // Process each temperature scenario (excluding T+10 which is already done)
  ...
  console.log(`\n✅ COMPLETE: Generated ${generatedCount} binary files`)
  console.log(`   ${dates.length} days × ${TEMPERATURE_SCENARIOS.length} temperature scenarios`)
  console.log(`   Each file includes color data for gradient visualization`)
}
...
async function main() {
  try {
    console.log('Modified Prediction Mesh Generator')
    console.log('==================================')
    ...
    // Process modified predictions
    await processModifiedPredictions(districtFeatures)

    console.log('\n✨ All meshes generated successfully with color data!')
  } catch (error) {
